...
Return ONLY the raw JSON array. Do not include markdown code block formatting or any other text.`;
  ...
  apiRouter.post("/sheets/sync", authenticate, async (req: any, res) => {
    try {
      ...
      // Robust RFC 4180-compliant CSV Parser that correctly handles:
      // - Multiline cells (newlines inside quotes)
      // - Commas inside quoted text
      // - Escaped double-quotes ("") within quotes
      ...
      for (let i = 1; i < allRows.length; i++) {
        ...
        // Capture every single column in its original spelling for detailed profile representation
