...
function logFirestoreWarning(message: string, error: any) {
  ...
  if (isPermissionDenied) {
    if (!isFirestoreServerDisabled) {
      ...
      console.log("Firestore server-side access disabled. Local fallbacks active.");
    }
  } else {
...
function logAuthWarning(message: string, error: any) {
  ...
  if (isPermissionDenied) {
    if (!isAuthServerDisabled) {
      ...
      console.log("Firebase Auth server-side access disabled. Local fallback active.");
    }
  } else {
    console.warn(`${message}: ${firstLineMsg}`);
  }
    ...
    try {
      ...
      if (error.code === "auth/user-not-found") {
        try {
          ...
          logAuthWarning("Could not create admin user", e);
        }
      } else {
  ...
  try {
    ...
    if (testDb) {
      testDb.collection("_temp_check_").limit(1).get()
        .then(() => console.log("Firestore connection test: ok"))
        .catch((err: any) => logFirestoreWarning("Firestore connection check failed during startup", err));
    }
  } catch (e) {
  ...
  apiRouter.post("/match", authenticate, async (req, res) => {
    try {
      ...
      const minimalExperts = experts.map((e: any) => {
        const name = e.name || e.Name || e.customFields?.Name;
        const expertise = e.expertise || e.role || e.customFields?.Expertise;
        const summary = e.summary || e.customFields?.Summary;
        const skills = e.skills || e.customFields?.Skills;
...
[
  {
    ...
    "matchScore": 95,
    "whyTheyFit": "2 sentences",
    "potentialGaps": "1 sentence"
  }
]`;
        ...
        parsed = experts
          .map((e: any) => {
            ...
            return {
              ...
              whyTheyFit: "Matched via keyword fallback system because the AI engine is currently unavailable.",
              potentialGaps: "Fallback matching lacks deeper contextual analysis."
            };
          })
      ...
      if (matches.length === 0 && experts.length > 0) {
        experts.slice(0, 3).forEach((e: any, idx: number) => {
          matches.push({
            ...
            potentialGaps: "May require adaptation to specific client context."
          });
        });
      }
  ...
  apiRouter.delete("/matches", authenticate, async (req: any, res) => {
    try {
      ...
      if (fs.existsSync(historyPath)) {
        ...
        history = history.filter((m: any) => m.userId !== req.user.uid);
    ...
    try {
      ...
      if (newExperts.length === 0) {
        return res.status(400).json({
          error: fetchError
            ? "Failed to access sheets. Please make sure 'Anyone with the link' can view them."
            : "No valid expert records found across provided sheets."
        });
      }
      ...
      try {
        ...
        fs.writeFileSync(configPath, JSON.stringify({ url, lastSynced: new Date().toISOString() }, null, 2), "utf8");
      } catch (e) {
