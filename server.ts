...
function getFirestoreDb() {
  if (isFirestoreServerDisabled) return null;
...
function getFirebaseAuth() {
  if (isAuthServerDisabled) return null;
  if (!admin.apps.length) return null;
...
Return ONLY the raw JSON array. Do not include markdown code block formatting or any other text.`;
      ...
      try {
        ...
        parsed = matchedIds.map(id => ({
          id,
          matchScore: 95,
          whyTheyFit: "Matched based on professional role alignment.",
          potentialGaps: "No obvious gaps identified."
        }));
      }
