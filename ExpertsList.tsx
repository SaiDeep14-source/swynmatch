...
export const ExpertsList = () => {
  ...
  const handleAddSheet = async () => {
    ...
    // Validate that the URL contains a real Google Sheets ID before saving.
    const urlTrimmed = newSheetUrl.trim();
    const sheetIdMatch = urlTrimmed.match(/\/d\/([a-zA-Z0-9_-]{20,})/);
    if (!sheetIdMatch) {
      alert(
        "Invalid Google Sheets URL.\n\nPlease paste the full URL from your browser address bar, e.g.:\nhttps://docs.google.com/spreadsheets/d/YOUR_SHEET_ID/edit\n\nMake sure the sheet is shared as \"Anyone with the link can view\"."
      );
      return;
    }
  ...
  const handleSyncSheet = async (sheetObj: SheetSource, skipSyncingReset = false) => {
    ...
    try {
      ...
      // Validate that we actually extracted a real Sheet ID (they are 44+ chars).
      // If the stored URL was never a real Google Sheets URL, sid will still be
      // the raw value (e.g. a single letter like "g" or "m"). Catch that early
      // and show an actionable error instead of a confusing proxy failure.
      if (sid.length < 20 || !sid.match(/^[a-zA-Z0-9_-]+$/)) {
        throw new Error(
          `Sheet "${sheetObj.name}" has an invalid URL. Please delete this source and re-add it using the full Google Sheets URL (e.g. https://docs.google.com/spreadsheets/d/YOUR_SHEET_ID/edit).`
        );
      }
